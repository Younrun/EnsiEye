...
events = [0, 0, 0]  # Lane, Distance, Sign
...
class VideoPage(Screen):
    ...
    def is_inside_roi(self, x_center, y_center):
        height = self.frame_height
        roi = np.array([[(200, height), (1100, height), (550, 250)]], dtype=np.int32)
        return cv2.pointPolygonTest(roi[0], (x_center, y_center), False) >= 0
    ...
    def update_frame(self, dt):
        ...
        for result in results:
            for box in result.boxes:
                ...
                distance = self.detector.get_depth_at(depth_map, cx, cy)

                if events[1] == 1:
                    if distance < 1.0:
                        cv2.putText(frame, 'Warning: Too Close!', (x1, y1 - 30), FONT, 0.7, COLORS['red'], 2)
                    if label in prev_distances and abs(prev_distances[label] - distance) < 0.05:
                        cv2.putText(frame, 'Slow Down', (x1, y1 - 50), FONT, 0.7, COLORS['yellow'], 2)

                    prev_distances[label] = distance
                    cv2.putText(frame, f"{label} {round(distance, 2)}m", (x1, y1 - 10), FONT, 0.6, COLORS['green'], 1)
                elif events[1] == 0:
